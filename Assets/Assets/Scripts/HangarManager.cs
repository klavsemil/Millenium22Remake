...
public class HangarManager : MonoBehaviour {
    ...
    public GameObject Grazer3; // for displaying a grazer in Hangar bay 02
    public GameObject Probe3; // for displaying a probe in hanagar bay 02
    public GameObject SIOS; // for displaying a probe in hanagar bay 01
    ...
    public void SetValuesInHangarWindow(int BayNumber, BaseItem ship) // this Method fill in the textfields on the hangarbaypanel for chosen vacant Bay
    {
        ...
        if(ship.ItemID==1 && BayNumber == 0 && ship.InProduction == false) // If this is a finished Grazer then display it in Hangar
        {
            Grazer.SetActive(true); // Show a grazer in Hangar 01 
        }
        if (ship.ItemID == 0 && BayNumber == 0) // If this is a probe then display it in Hangar
        {
            Probe.SetActive(true); // Show a ptobe in Hangar 01 
        }
        if (ship.ItemID == 4 && BayNumber == 0) // If this is a SIOS Base then display it in Hangar
        {
        ...
        if (ship.ItemID == 1 && BayNumber == 1) // If this is a Grazer then display it in Hangar
        {
            Grazer2.SetActive(true); // Show a grazer in Hangar 02             
        }
        if (ship.ItemID == 0 && BayNumber == 1) // If this is a probe then display it in Hangar
        {
            Probe2.SetActive(true); // Show a probe in Hangar 02             
        }


        if (ship.ItemID == 1 && BayNumber == 3) // If this is a Grazer then display it in Hangar
        {
            Grazer3.SetActive(true); // Show a grazer in Hangar 03             
        }
        if (ship.ItemID == 0 && BayNumber == 3) // If this is a Probe then display it in Hangar
        {
